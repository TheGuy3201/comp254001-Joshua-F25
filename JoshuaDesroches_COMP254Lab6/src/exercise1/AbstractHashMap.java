...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  protected double maxLoadFactor;      // maximum load factor before resize
  ...
  /** Creates a hash table with given capacity, prime factor 109345121, and default load factor 0.5. */
  public AbstractHashMap(int cap, int p) { this(cap, p, 0.5); }

  /** Creates a hash table with given capacity and prime factor 109345121, default load factor 0.5. */
  public AbstractHashMap(int cap) { this(cap, 109345121); }  // default prime

  /** Creates a hash table with capacity 17, prime factor 109345121, and default load factor 0.5. */
  ...
  /** Gets the current load factor (n / capacity). */
  ...
  public V put(K key, V value) {
    ...
    if (getCurrentLoadFactor() > maxLoadFactor) {
      // resize to roughly double the capacity while maintaining reasonable new size
      int newCap = (int) Math.ceil(2.0 * capacity / maxLoadFactor);
      resize(newCap);
    }
    return answer;
  }
