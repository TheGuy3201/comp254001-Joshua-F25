...
class MergeSort {
  ...
  /**
   * Bottom-up merge-sort for a collection of items by placing each item in its own queue,
   * and then repeatedly merging pairs of queues until all items are sorted within a single queue.
   * Uses a queue of queues to manage the merging process.
   */
    public static <K> void mergeSortBottomUp(Queue<K> S, Comparator<K> comp) {
        int n = S.size();
        if (n < 2) return; // Queue is already sorted

        Queue<Queue<K>> queueOfQueues = new LinkedQueue<>();

        // Step 1: Place each item in its own queue and enqueue it to the queue of queues
        while (!S.isEmpty()) {
            Queue<K> singleItemQueue = new LinkedQueue<>(); // Create a new queue for the single item
            singleItemQueue.enqueue(S.dequeue()); // Enqueue the item from S
            queueOfQueues.enqueue(singleItemQueue); // Enqueue the single-item queue
        }

        // Step 2: keep merging merge pairs of queues until only one queue's left
        while (queueOfQueues.size() > 1) {
            int currentSize = queueOfQueues.size(); // Number of queues to process in this round

            // Process pairs of queues in this round
            for (int i = 0; i < currentSize / 2; i++) {
                Queue<K> Q1 = queueOfQueues.dequeue(); // Dequeue the first queue
                Queue<K> Q2 = queueOfQueues.dequeue(); // Dequeue the second queue
                Queue<K> mergedQueue = new LinkedQueue<>(); // Create a new queue for the merged result
                merge(Q1, Q2, mergedQueue, comp); // Merge Q1 and Q2 into mergedQueue
                queueOfQueues.enqueue(mergedQueue); // Enqueue the merged queue back to the queue of queues
            }
        }

        // Step 3: The final sorted queue is the only remaining queue in the queue of queues
        if (!queueOfQueues.isEmpty()) {
            Queue<K> sortedQueue = queueOfQueues.dequeue(); // Dequeue the final sorted queue
            while (!sortedQueue.isEmpty()) {
                S.enqueue(sortedQueue.dequeue()); // Enqueue elements back to S
            }
        }
    }
}
