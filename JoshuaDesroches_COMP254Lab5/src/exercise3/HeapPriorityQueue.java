...
import exercise3.requiredFiles.AbstractPriorityQueue;
import exercise3.requiredFiles.Entry;
...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  protected void upheapRecursive(int j) {
    if (j > 0) {            // continue until reaching root
      int p = parent(j);
      if (compare(heap.get(j), heap.get(p)) < 0) { // heap property violated
        swap(j, p);
        upheapRecursive(p); // recur at the parent's location
      }
    }
  }
